...
public sealed class DiskAnalyzer(IQuarantinePathCheck? quarantine = null)
{
    ...
    private static ScannedFile? Describe(string path)
    {
        try
        {
            ...
            // Синхронизируемая папка: удаление отсюда уходит на все устройства
            // пользователя, и карантин этого не отменит. Признак ставится здесь,
            // а решение принимает охрана — она поднимет такому файлу уровень до красного.
            if (CloudFolders.Contains(path))
            {
                traits |= FileTraits.InCloudFolder;
            }
