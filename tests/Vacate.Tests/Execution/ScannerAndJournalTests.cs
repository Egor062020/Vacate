...
public sealed class TempFilesScannerTests : IDisposable
{
    ...
    {
        _sandbox = Path.Combine(Path.GetTempPath(), "Vacate.Scanner.Tests", Guid.NewGuid().ToString("N"));
...
public sealed class JournalTests : IDisposable
{
    ...
    public JournalTests()
    {
        _directory = Path.Combine(Path.GetTempPath(), "Vacate.Journal.Tests", Guid.NewGuid().ToString("N"));
